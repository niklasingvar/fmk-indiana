...
- Casablanca is Electron-only. Run `cd crates/casablanca && npm run dev` and verify the separate Electron window it opens. NEVER open `http://localhost:5173` (or 5174, etc.) in a browser or browser tool — that URL is the renderer dev server only, it has no `window.api`, and it always shows "Preload bridge missing". That message in a browser tab is expected, not a bug. If the Electron window itself reports a missing preload bridge, inspect its DevTools and preload path. Do not add browser mocks unless browser support is explicitly requested.
