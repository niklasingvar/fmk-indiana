...
- Stage panels (mutually exclusive, top-right icons): Properties, Indianas, Tasks, Agent runs, History — History is the existing per-note git history panel; no repository-wide Git panel yet
...
- Agent runs panel (stage bolt icon): run history from `.indiana/chief-of-staff/runs/` records via `indiana runs --json` (grammar in Rust only) — list newest first with outcome dot, tokens, cost; selecting a run shows structured facts plus the transcript body (read raw for display); refreshes when a new record lands — `renderer/src/cos/RunsPanel.tsx`, `main/lib/agent-runs.ts`, [COS_PRD.md](../chief-of-staff/COS_PRD.md)
